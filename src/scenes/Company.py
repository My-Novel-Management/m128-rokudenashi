...
def escaping(w: World):
    ...
    return w.scene('脱出',
            ...
            dad.talk("何だ？"),
            masa.talk("いや"),
            masa.think("本物の消防士みたいだと思った、なんてとても口にはできない"),
...
def safely_escaped(w: World):
    ...
    return w.scene("無事脱出したが",
            ...
            masa.think("外だ", "やっと死地から抜け出たのだ"),
            man.talk("大丈夫ですか！"),
            masa.do("コンクリートの地面に両手を突いた$Sに、銀色の消防服を身にまとった隊員が駆け寄って声を掛ける",
                "それに「はい」と答えるのが精一杯で、歪んだ視界には十台以上の消防車と救急車が並び、各員が忙しそうに行き来しているのが映った"),
            masa.do("振り返れば既に建物は全体が炎に包まれ、奥の研究所の棟は三階部分が完全に崩落しているのが分かった",
                "残っていたら完全に助からなかった",
